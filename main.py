...
# only keep things you would actually see in street traffic. this drops odd
# misreads like the roadside statue the model called a surfboard.
...
def annotate(image, detections, names):
    ...
    # keep the label and score of everything I draw, to print a summary later
    found = []
    for x1, y1, x2, y2, conf, class_id in detections:
        label = names[int(class_id)]

        # skip anything that is not a street object
        if label not in STREET_CLASSES:
            continue
